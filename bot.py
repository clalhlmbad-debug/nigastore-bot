...
# التوكن الخاص ببوتك (Niga Store)
TOKEN = "8826744317:AAEhuJxpjx8m9il7rwvEJmca75qrJOvJ55M"
bot = telebot.TeleBot(TOKEN)

# عنوان الشام كاش الفعلي الخاص بك
SHAM_CASH_ACCOUNT = "df910e178e027a6bfcae8b99b06b5384"

# مخزن مؤفت لحفظ بيانات طلبات الزبائن قيد التنفيذ
user_orders = {}
...
def send_welcome(message):
    ...
    welcome_text = (
        f"🎯 أهلاً بك يا {message.from_user.first_name} في بوت Niga Store!\n\n"
...
def callback_query(call):
    if call.data == "games_menu":
        ...
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, 
                              text="🎮 مرحباً بك في قسم الألعاب! اختر لعبتك المفضلة:", reply_markup=markup)
    ...
    # بدء معالجة الشراء عند الضغط على فئات ببجي وفري فاير
        ...
        # حفظ فئة الشراء مؤقتاً للزبون الحالي
        ...
        markup = InlineKeyboardMarkup()
        markup.add(
            InlineKeyboardButton("💬 تواصل مع المطور", url="https://t.me"),
            InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="main_menu")
        )
        support_text = (
            "🛠️ **قسم الدعم الفني والمساعدة**\n\n"
            "إذا واجهتك أي مشكلة أو كان لديك استفسار حول شحن الألعاب، "
            "يمكنك التواصل مباشرة مع الإدارة عبر الضغط على الزر أدناه."
        )
...
# استقبال الرسائل النصية المكتوبة من الزبون (الـ ID ورقم المعاملة)
...
def process_order_steps(message):
    ...
    # الخطوة الأولى: استلام الـ ID من الزبون
    if step == "get_id":
    ...
    # الخطوة الثانية: استلام رقم معاملة الشام كاش وإنهاء الطلب
        ...
        # حذف البيانات المؤقتة بعد إتمام الطلب بنجاح
        del user_orders[user_id]

print("البوت يعمل الآن بالنسخة المستقرة السابقة...")
