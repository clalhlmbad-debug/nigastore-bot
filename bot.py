...
TOKEN = os.environ.get("BOT_TOKEN", "8826744317:AAHR9wuT8sNK0Vg98uCcGmaps7-YntrSWiQ")
ADMIN_ID = int(os.environ.get("ADMIN_ID", 8192730669))
SHAM_CASH_ACCOUNT = os.environ.get("SHAM_ACCOUNT", "df910e178e027a6bfcae8b9b06b5384")
EXCHANGE_RATE = int(os.environ.get("EXCHANGE_RATE", 145))
...
def callback_query(call):
    ...
    if call.data == "games_menu":
        ...
        markup.add(
            types.InlineKeyboardButton(f"💳 بطاقة غوغل 5$ - {round(PRICES['buy_card_google_5'] * EXCHANGE_RATE)} ل.س", callback_data="buy_card_google_5"),
            types.InlineKeyboardButton(f"💳 بطاقة غوغل 10$ - {round(PRICES['buy_card_google_10'] * EXCHANGE_RATE)} ل.س", callback_data="buy_card_google_10"),
            types.InlineKeyboardButton(f"💳 بطاقة غوغل 25$ - {round(PRICES['buy_card_google_25'] * EXCHANGE_RATE)} ل.س", callback_data="buy_card_google_25"),
            types.InlineKeyboardButton(f"💳 بطاقة غوغل 50$ - {round(PRICES['buy_card_google_50'] * EXCHANGE_RATE)} ل.س", callback_data="buy_card_google_50"),
        ...
        bot.answer_callback_query(call.id, "⚠️ عذراً، هذه الفئة غير متوفرة في المتجر حالياً!", show_alert=True)
